...
\begin{enumerate}
	...
	\item
		...
		\vspace{0.001em}\\
		\item $\hat{p}_H$, $\bar{p}_H$ vs Trial Size
		\begin{figure}[h]
		\includegraphics[scale=0.75]{hw2_ques2_2_d}
		\end{figure}
		$\bar{p}_H$ is the observed ratio of heads to the total number of flips.  It performs better than $\hat{p}_H$ in the case where our variance is very low (0.1) and constant since a low variance means that noise is going to be very close to 0. As you increase the variance,  $\hat{p}_H$ does better, and converges to $p_H^*$ as the trial size increases.
		TODOTODOTODOTODOTODO!!!!!!
		\vspace{0.001em}\\
		\end{enumerate}
		\item
